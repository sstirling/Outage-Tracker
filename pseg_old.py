...
for county in counties:
    ...
    for member in members:
        ...
        cur.execute("INSERT INTO outages_pseg(company,updated,timestamp,county,muni,outage) VALUES(%s,%s,%s,%s,%s,%s)",(row[0],row[1],row[2],row[3],row[4],row[5]))
        conn.commit()
        cur.execute("INSERT INTO outages_summary(timestamp, pseg) VALUES(%s, %s)",(row[2],row[5]))
        conn.commit()
