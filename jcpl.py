...
for county in counties:
    ...
    for muni in munis:
        ...
        cur.execute("INSERT INTO outages_jcpl(company,updated,timestamp,county,muni,outage,customers) VALUES(%s,%s,%s,%s,%s,%s,%s)",(row[0],row[1],row[2],row[3],row[4],row[5],row[6]))
        conn.commit()
        cur.execute("INSERT INTO outages_summary(timestamp, jcpl) VALUES(%s, %s)",(row[2],row[5]))
        conn.commit()
